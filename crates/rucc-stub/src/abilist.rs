...
//!
//! Design: `spec/cross-compile/09-libc-stubs.md` section 9.2, which calls this the artifact that
//! makes the whole scheme feasible.
//!
...
//! change is reviewed, and it is text, which matters both for document 13's size budget and for
...
/// 9.2's compressed description, or from a string in a test, and none of that is this module's
        ...
        let kind = match letter {
            ...
            "O" => {
                return Err(Error::Unwritable {
                    line: line_number,
                    letter: 'O',
                    what: "a powerpc64 function descriptor, and the `.opd` handling it wants is \
                           not settled here",
                });
            }
