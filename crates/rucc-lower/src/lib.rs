...
//!
//! What it does not build yet is reported rather than mislowered. A computed `goto`, inline
//! asm, a `goto` in a function that has a variable length array in it and a `va_arg` that reads
//! a structure each become a diagnostic, so a program that uses one fails to compile rather
//! than compiling into something that is not what it says.
//!
//! Every crate in the workspace is published, and publishing implies a promise. This one is
//! tier 3: its Rust API is explicitly unstable and will change without a major version bump.
//! Depend on the `rucc` binary's behaviour, not on this.

#![doc(html_root_url = "https://docs.rs/rucc-lower/0.2.14")]
