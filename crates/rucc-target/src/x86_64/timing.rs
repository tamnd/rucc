...
fn plain(form: Form) -> Timing {
    ...
    let (latency, unit) = match form {
        ...
        // What is left: a fence, a trap, a landing pad, a pad and a spin hint. None of them
        // produces a value anything waits on, and every one of them is in the function for a
        // reason its operands do not say, which is what `Unit::Fixed` is and what stops a schedule
