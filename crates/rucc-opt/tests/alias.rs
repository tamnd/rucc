...
use rucc_opt::{Access, Alias, Answer, Outside, Reason};
...
fn a_no_is_only_ever_said_about_two_references_that_really_do_not_meet() {
    ...
    for _ in 0..400 {
        ...
        let mut alias = Alias::new(&func, &outside);
        ...
        let mut again = Alias::new(&func, &outside);
        for (a, _) in &accesses {
            for (b, _) in &accesses {
...
impl Case {
    ...
    fn build(&self) -> (Module, Func, Vec<(Access, Truth)>) {
        ...
        let outside = Outside::of(&module);
        let alias = Alias::new(&func, &outside);
        let accesses = out
            .into_iter()
