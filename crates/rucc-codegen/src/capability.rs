...
pub static HAND: &[(Opcode, &str)] = &[
    ...
    (Opcode::Apply, "`crate::lower`, as a call built out of that block"),
    ...
    // The anonymous arguments of a call that was inlined, which the inliner puts in its place, and
    // a body still holding one is a body that is not emitted.
    (Opcode::VaArgPack, "nothing at all, since `rucc_opt::inline` replaces it first"),
    (Opcode::VaArgPackLen, "nothing at all, since `rucc_opt::inline` replaces it first"),
