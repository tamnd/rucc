...
mod tests {
    ...
    fn usage_fits_on_a_screen() {
        ...
        // reading this list will see them. The one it went up by last is the sysroot, which is the
        // question somebody asks when a cross build read a file nobody expected, and which has no
        // room on the line above it because the answers there are a path each and this one is the
        // root all of them are under.
        assert!(USAGE.lines().count() < 58, "usage text has grown past one screen");
    }
}
