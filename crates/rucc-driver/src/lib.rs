...
mod tests {
    ...
    fn usage_fits_on_a_screen() {
        ...
        // the same line as three flags that change nothing would be misleading about both. The two
        // it went up by last are the prefix mapping family, which is four flags whose whole job is
        // to keep a build's output the same from two different directories, and which a person
        // chasing a reproducible build comes here looking for by name.
        assert!(USAGE.lines().count() < 61, "usage text has grown past one screen");
    }
}
