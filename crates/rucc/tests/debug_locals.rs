...
//!
//! Only the locals lowering gave a frame slot are here, which is the ones whose address is taken
//! and the ones too big for a register. A scalar the program never took the address of is held in
//! an SSA value, and saying where one of those is needs a location list over the register
//! allocator's output, which is the rest of tamnd/rucc#9. That is why the fixture below takes an
//! address: a function of nothing but scalars would produce a correct object with no locations in
//! it at all, and a test written over one would pass without the wire.
//!
