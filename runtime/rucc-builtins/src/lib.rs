...
//! Double precision is most of the way there, with the four operations, the negation and the eight
//! comparisons on a `double` in `double`, which is the same shape one format up and two routines that
//! are genuinely new, a product that no longer fits in a word and a division that cannot be written
//! as a `/`. The rest of section 12.8, which is the conversions at that width, the pair that widens
//! and narrows between the two formats, quad soft float, the remaining `__int128` arithmetic, the
//! eighty bit and quad conversions and the atomics, is not written, and the set is driven by what
//! the target ladder in `spec/14-target-ladder.md` actually calls.
