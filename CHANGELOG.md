...
There is no `#pragma once` and no multiple include optimization, so a header with an ordinary include guard is read again every time it is named rather than being skipped on sight. `#embed` is recognised and refused with a diagnostic rather than silently ignored, because it needs the parser. `#line` is recorded and not applied. Those, the header cache and the predefined macro set are the next piece of M1.
