...
//! the seven checks that compile C programs and run them are each one process doing one thing at a
...
//!
//! The seven checks that compile C and run it all want `target/release/rucc`, so they cannot start
...
//! themselves each would ask cargo to build it, and seven of those waiting behind the tests for
...
enum Verdict {
    ...
    /// every check as a process. A machine with no container cannot run the seven that compile C
...
/// pass it. The compiler second because it is what the seven checks after this need and nothing
...
/// it is worth the contention: they are one process each doing one thing at a time, so seven of
...
fn programs() -> Result<Vec<Step>> {
    ...
    // while the tests hold the build directory's lock is seven waits for a build that already
    ...
    ["unwind", "quad", "wide", "fuzz", "safety", "dso", "accounting"]
        .iter()
...
pub(crate) fn ci() -> Result<()> {
    ...
    let (documented, walked, ran, built) = std::thread::scope(|scope| {
        ...
        let ran = scope.spawn(move || match hear.recv() {
            ...
            // in both cases the failure is the spine's to report rather than seven more of it.
            _ => Ok(Vec::new()),
        });
