...
mod tests {
    ...
    /// all.
    ///
    /// Three files rather than one, because a conversion between two formats lives with the narrower
    /// of the two: `quad.c` has the arithmetic, the comparisons and the conversions against an
    /// integer, and the four that cross to a `float` or a `double` are in `float.c` and `double.c`.
    /// `spec/12-abi-and-runtime.md` section 12.8 says why.
    #[test]
    fn every_routine_in_the_list_is_one_the_runtime_defines() {
        let builtins = root().join("runtime").join("builtins");
        let mut text = String::new();
        for file in ["quad.c", "float.c", "double.c"] {
            text.push_str(
                &std::fs::read_to_string(builtins.join(file))
