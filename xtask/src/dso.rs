...
pub(crate) fn dso() -> Result<()> {
    ...
    println!("dso: {} checks, one library, {runner}", WANTED.len());
    Ok(())
}

/// Compiles the library with this compiler and lays out the directory the runner is pointed at.
///
/// `-fPIC`, which is the whole of what this is about. Assembly rather than objects so that a
/// failure is something a person can read, and so that the assembler in the container is the one
/// that has to agree with the listing rather than one more thing this compiler is trusted about.
fn build() -> Result<PathBuf> {
    ...
    for name in OURS {
        let out = Command::new(&rucc)
            .args(["-S", &format!("--target={TRIPLE}"), "-fPIC", "-O1"])
            .arg("-o")
            .arg(work.join(format!("{name}.s")))
...
fn read(printed: &str) -> BTreeMap<String, String> {
    ...
    for line in printed.lines() {
        if let Some(rest) = line.strip_prefix("dynsym ") {
            ...
            if fields.len() >= 8 && WATCHED.contains(&name) {
                said.insert(format!("dynamic symbol {name}"), visibility.to_owned());
            }
            continue;
        }
