...
What GCC's way comes to is that `defined` is an operator wherever it turns up on a `#if` line, so expansion leaves it and the name it asks about alone and the pass that answers it runs again afterwards. Leaving the name alone is the whole of it: a macro that is defined as `1` would otherwise be asked about `1`, and one that is defined as nothing would leave `defined()` and an error. mingw-w64's `<psdk_inc/intrin-impl.h>` writes `(!defined(__INTRINSIC_DEFINED_ ## name))` and is reached from `<windows.h>`, so this is the difference between having the platform's headers and not. The warning is `W0334` and its wording is GCC's, with the chain of macros the operator came out of under it.
...
**Multiple-include optimization** is not optional for performance. A file whose entire content is wrapped in `#ifndef GUARD / #define GUARD / ... / #endif` is recorded with its guard symbol, and subsequent includes are skipped without opening the file once the guard is defined. `#pragma once` does the same. Both are recorded against file identity rather than against the name the include used, because one header is reached under several names on any project with more than one include directory. The file system supplies the identity, so a preprocessor running on a map from name to bytes still has an answer; the implementation on a real file system canonicalizes, which resolves the relative part, the `..` and the symlinks. Two hard links to one file are two identities under that rule where GCC, which compares device and inode, sees one. `#pragma once` in the main file is applied as well as warned about, which is what GCC does and what a file that includes itself needs. The whole content means the whole content: a token outside the conditional stops it being a guard, and so does another branch of that same conditional, since a file whose opening `#ifndef` has an `#elif` or an `#else` behind it has something to produce on the second read and is not a file that can be skipped. A branch nested inside the guard is a different thing and says nothing about the file, so the two are told apart by conditional depth rather than by the directive's name. That is GCC's rule as well, and the file that made the point is the one tcc's suite includes three times under three spellings of its name to check that computed includes work.
...
A macro defined twice warns when the two definitions differ and says nothing when they are the same, which is the standard's rule and is not the interesting part. The interesting part is that in a hosted translation unit two of the definitions are not written by the program. The built in set is one, and on a glibc target `<stdc-predef.h>` is read before the first line of the file and writes several of the same names again from the library's side. So the built in values are not only a claim about this compiler, they are one half of an agreement, and a value that is defensible on its own and different from the library's produces a warning on a line nobody wrote. `__STDC_IEC_60559_BFP__` is the one that caught us: C23 gives the macro its own date, glibc writes TS 18661-1's, and the only way for a program to see no warning is for this compiler to write what GCC writes. Where the two documents disagree about a name that `<stdc-predef.h>` also defines, GCC's value wins.
