...
`-fPIC`, `-fpic`, `-fPIE` and `-fpie` are accepted and change nothing, because position independent code is the only kind this compiler emits. An address that may turn out to live in another object is loaded out of the global offset table rather than worked out from where the instruction is, `__PIC__` and `__pic__` have been defined as `2` on every target that has a loader since the predefines were written, and section 11.3 has the relocation that comes of it. Accepting these matters more than the code they generate, which is none: every autoconf and cmake project puts `-fPIC` on the compile line, so a compiler that refuses it cannot be the `CC` of anything with a configure script whatever else it can do. `-fno-pic` and `-fno-pie` are the other direction. They are a request rather than a description and not one this compiler can grant, so they are refused with a message that says why, per section 13.3, rather than accepted and ignored. Compiling them as position independent anyway would be answering a different question: the answer is right everywhere an ordinary program runs, and it is wrong in a kernel, which is the place the flag gets written because there is no loader to fill a global offset table in.

Code generation flags: `-fno-plt`, `-fno-omit-frame-pointer`, `-fomit-frame-pointer`, `-fstack-protector` and its `-strong`/`-all` variants, `-fstack-clash-protection`, `-fcf-protection=`, `-fno-common`, `-ffunction-sections`, `-fdata-sections`, `-fvisibility=`, `-mcmodel=`, `-mno-red-zone`, `-fpatchable-function-entry=`, `-mfentry`, `-pg`. The last several are kernel requirements and are specified in document 13.
