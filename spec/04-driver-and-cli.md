...
Individual `-f<pass>` and `-fno-<pass>` flags exist for every pass, which is required for bisection: when a project miscompiles at `-O2`, the first diagnostic step is to find the pass. `-fdisable-<pass>[=<functions>]` and `-fenable-<pass>[=<functions>]` take a pass and, optionally, the functions the answer is about, and `-fpass-fuel=<pass>=<n>` runs a pass for exactly *n* transformations and then stops, which is how a miscompiling transformation is bisected to a single site. Document 15 depends on all three.
