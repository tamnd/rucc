...
What is left out of the line for now is `crtbegin.o`, `crtend.o` and `-lgcc`. All three belong to the compiler's own runtime, which is document 12's `rucc-builtins` and so far has only its block routines, and a Linux program that does not need a helper routine or a static constructor links and runs without them. They go in when there is something to put in them, and the corpus is what will say we did not break the line putting them there. The place their absence shows first is `-static`, which fails on the unwinder that `libc.a` refers to and a shared `libc` resolves inside itself, so a static link waits on the runtime along with the rest.
