...
**What that means for the design.** Tier E is 1.74x to 1.80x, not 1.3x, and document 02.6's first failure condition is the live one. The response is not another source of facts. Three of them together move the clock by 7 percent and a fourth would move it by less. It is the loop rows `split` still refuses, which `cargo xtask` reports and tamnd/rucc#810 leads: 3364 checks at 332 sites where the address is not something the analysis can follow, 662 where a call in the loop might free what it reads, 598 in a loop inside the one being split. The first of those is no longer one row. It is reported as the shapes it is made of, and about two fifths of it is a walk over a linked structure, which is a case this pass will never take rather than one waiting on better analysis. Those are checks in hot code that nothing discharges, and they are where the remaining 0.5x is.
...
**What the second corpus member showed, and it is not the same thing.** zstd is the fifth row of `cargo xtask libraries` and it does not fit either side of the argument above. It keeps every position in the compressor as a 32 bit index rather than as a pointer, and it keeps one pointer that turns an index into an address: `ZSTD_window_update` in `lib/compress/zstd_compress_internal.h` sets `window->base = ip - distanceFromBase`, which is the caller's buffer moved backwards by everything the compressor has seen so far and is a long way below the start of any object after the first few blocks. That pointer is never dereferenced. Every match finder the library has then reads through `base + matchIndex`, and the result is back inside the buffer the base was derived from. One run of the workload, which is three compressions of two hundred kilobytes, one streamed compression and a dictionary trained from three hundred samples, refuses 997 derivations and 4,710,697 reads through what they handed back, at 39 distinct sites, and the program gets every answer right and exits 0. Not one of those reads touches a byte outside any object.

So the two corpus members ask opposite things. SQLite's four are derivations nothing ever reads through, where refusing early reports something and prevents nothing. zstd's are derivations everything reads through, where refusing early reports 997 times and refusing at the access instead would report 4.7 million times. The candidate answer below is better for SQLite and much worse for zstd, and neither of them is a library doing anything wrong.
...
**The third answer, which is the one zstd is asking for.** Let a pointer leave its object and come back, and judge the access rather than the excursion: a derivation outside the object keeps the capability instead of losing it, so a read through `base + i` is permitted exactly when `base + i` is inside the object `base` names, and nothing is reported for `base` itself. That is what C does not allow and what every real compiler does anyway, and it is the difference between the model covering zstd and the model requiring zstd to be rewritten. It gives up catching a derivation early, which is the thing document 03.1 argues for, and it means a pointer that walks off the end and keeps going is caught only when it lands somewhere. It is also not something hardware has already solved: on CHERI zstd's base is outside the representable range and gets untagged, which is why that library carries patches there.

**What would settle it:** classify the S5 refusals across more than one corpus member, which is now two and should be more. SQLite gives four false alarms to zero catches, and no amount of report quality pays for a check that only ever stops correct programs. zstd gives 997 to zero and would give 4.7 million under the candidate answer. If a member turns up whose S5 refusals are on pointers that are then written through somewhere they should not be, the early report is doing the work document 03.1 says it does and the answer is a narrower exemption rather than any of these three. Either way a middle answer exists and should be measured alongside: keep S5 as a refusal, add a mode that demotes it to a logged warning carrying both the derivation site and, if an access ever happens, the access site, and see whether the demoted report is still good enough to find a bug with. A second middle is to keep the capability across an excursion but bound how far it may travel, since a model that keeps the bounds in a word has no reason to lose them and hardware that compresses them does.
