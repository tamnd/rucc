...
**One loop, both kinds of access.** A loop that walks an array and reads a header field on every iteration has an address that moves and an address that does not, and the two want different arithmetic. The moving one has a limit, which is how many iterations fit in the room the extent leaves, and the still one has an answer, which is yes or no. The plan's limit is the smallest of what the moving ones allow, taken only when every still one fits, and a still one that does not fit sends the whole loop down the checked half rather than putting a bound on it, because an access that would fail fails on the first iteration as much as on the last. Deciding this once for the plan rather than once per access is what tamnd/rucc#818 was: a still access has a step of zero, and asking it how many iterations it allows divides by that step.
