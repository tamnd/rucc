...
**The other two planes.** Everything above is written about the bounds check and applies word for word to the type check and the init check, and the reason is one property all three share: each is a claim about a range that holds of every subrange of it. Every byte from here to there is inside the object, every granule from here to there agrees with this type, every byte from here to there has been written. Take a piece out of the middle of any of those and the same sentence is true of the piece, which is the whole of what the transformation needs, so the rule proved for the bounds check is the rule the other two ask and there is no second proof obligation. The runtime side is free as well, since the calls the two plane checks lower to already take a width, so the range query the hoisted check wants is the call that was already there with a larger number in it. One condition is new. A loop cannot move the edges of an object without calling something, which is why the bounds check has nothing to fear from the loop's own body, but ordinary code writes the other two planes: a store writes the init plane and an assignment through a pointer of a different type writes the type plane. So `a[i] = a[i - 1]` over an array with only `a[0]` written is a loop every iteration of which passes its init check and which one check in front of the loop would refuse, and a loop that writes either plane has to keep that plane's checks. The last thing worth recording is what the hoisted check does not see. The runtime clips the range to the region the first address is in, so a walk that runs off the end asks about fewer bytes than the number handed in, and the accesses past the end are reported by the bounds check rather than by the plane check. That is under reporting only where the bounds plane is off and one of the other two is on, and it is worth writing down rather than worth refusing over.

**Where the walk starts.** The check in front has to be written at the address the first iteration reads, and for a while that address had to be a pointer plus a number, which is the walk over a whole array and is not the walk over a row of one. `a[i * N + k]` round `k` starts at `a` plus `i * N` elements, and `i * N` is something the loop does not change but it is not something the compiler has as a number, so the whole loop was refused and a matrix multiply kept every check it had. It does not have to be a number. It has to be something the block in front of the loop can work out, and an expression built on values the loop does not change is exactly that, so the offset past the anchor is carried as the expression and written into the preheader: widen the value the way the source read it, multiply, add, and step the anchor along by the result. Every part of that is left out where the numbers make it nothing, so the array walk still writes no arithmetic and the row walk writes three instructions in a block that runs once. Nothing in the argument changes, because the rule is about a distance from wherever the walk starts and the hoisted check takes its capability from the address it is written at. What does change is the capability condition: a start the preheader has to work out is a start further along, further along may be inside some other object, and that is the same sentence the constant offset case was already saying. So a bounds check whose capability belongs to the array rather than to the access keeps its place, and the two plane checks do not read the capability at all, which is why this is worth doing for them first.

**What the range in front costs.** Widening what comes out of a loop turned up a thing worth writing down, which is that the three checks do not all cost the same amount per byte of range. A bounds check is two comparisons whatever range it is handed, so one of them in front of a loop is one of them and how far the walk reaches never enters into it. The two plane checks answer by reading the plane over the range, so what they cost goes up with the range rather than with what the loop reads, and a walk that leaves gaps hands them bytes nobody touched. `b[k * N + j]` round `k` is the shape that says so: two hundred reads of eight bytes each with three hundred and eighteen kilobytes between the first and the last, and one plane check over the lot of it was measured at thirty three times the whole program. So a plane check comes out of a loop only where the step is no wider than the access, which is to say only where the range in front is the bytes the loop reads rather than the ground it covers, and a bounds check keeps the old condition, because for it the two are the same price. The other way to read that measurement is that the runtime's ranged plane check is linear in the range where a per element one is not, which is a question about the runtime rather than about this pass, and it is worth asking separately.
