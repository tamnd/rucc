...
**Second instalment, from the same program.** A third of the four queries is invariant in the loop one out rather than in both, and that one did not move either, silently, because the chain between it and the front of the loop starts with a sign extend. A sign extend is cheap, the loop was under pressure, and `licm` refuses a cheap computation under pressure before it knows what is going to read it, so the expensive query behind it was never a candidate at all. Letting a cheap link ride along and taking it back out if nothing boarded behind it takes the multiply to 356648839 instructions with 15221423 in `__rucc_extent`, so the four queries per entry to the innermost loop are now one. The other seven programs move by less than a hundredth of a percent, which is the honest shape of it: this is one program's chain and the general statement is that a register pressure answer given before the readers are known is a guess.

That change puts the pressure the other way. `cargo xtask pressure` goes from 79 slots and 337 stack moves with the monitor on to 89 and 351, against 28 and 95 with it off, so the hoist costs ten more slots across the eight programs and buys 16464987 instructions on one of them. Both numbers are real and the instruction count is the one that decides, because a slot is static and a reload in a loop three deep is not. The accounting behind the slot number is itself wrong in a way that is worth fixing separately and is on #898: a chain of ten whose intermediates nothing else reads is charged ten registers and holds one.

**A ninth program, and what the allocation path costs.** The tables above are eight programs and the set is nine now. Every one of the eight allocates what it is going to walk and then spends its time walking it, so the per instance work is amortized away long before the ratio is taken, and none of them can see a change to what an allocation does. `an-allocate-and-free-loop` allocates a block, writes two bytes of it, frees it, and does that 1.6 million times over eight size classes, which is what a program built out of short lived objects looks like and is where the per instance clears are paid for.

The thing it was added to measure is the aux clear at reuse. A block off a free list carries the previous tenant's slots, so an instance beginning has to clear them, which is two bytes of metadata per byte of payload written on the allocation path. Measured by relinking the same object file against two runtimes and alternating the runs, it is 4 to 5 percent on that program: 0.725 s against 0.695 s at the median of twelve, 0.67 against 0.64 at the minimum. On a program that does almost nothing else, which is the point of it. That is also the argument for clearing only on reuse: a block off the bump has never been anyone's and the storage behind the bump is a fresh mapping, so clearing it would pay the same 4 percent for nothing.
