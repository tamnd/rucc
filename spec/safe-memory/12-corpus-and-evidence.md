...
**Six of these are built and run under the monitor today**, which is `cargo run -p xtask -- libraries`: SQLite, zlib, Lua, brotli, zstd and libwebp. Each of the six compiles from its own unmodified sources at `-O0` and at `-O2`, links against the instrumented runtime, and runs a workload under `tests/` that checks its own answers by arithmetic it does itself, so the answers are the same numbers against any version of the library. Every report the monitor makes is written down in the row with why the library is entitled to it, and an unlisted report fails the check, because these workloads do nothing wrong. zstd's two reports are the exception and the row says so: they are question 12 of document 17 arriving with a library behind it rather than a test case. libwebp's seven are the widest set any row has and they are two idioms rather than seven: an SSE load that takes a lane nobody wrote because it takes sixteen bytes at a time, which is zlib's `slide_hash` again, and a run of pixels the lossless decoder fills through a `uint64_t*` and reads back through a `uint32_t*`, which is an effective type the library changes on purpose and gets away with.
