...
**What a copy does with the metadata.** A copy is the one wrapper that moves metadata rather than making it, and it moves all of it. The type plane carries the source's answer across rather than recording a character write, which is C 6.5's rule and is what keeps punning through a character buffer working. The init plane carries the source's answer too, so padding a program never filled is still padding when it arrives somewhere else, which is document 09 section 9.3's infoleak. The aux carries as well: a structure holding pointers arrives with the pointers in it, and what says where each one points lives in the slot beside the word rather than in the word, so a copy that moved the words and left the slots would describe the destination's pointers with whatever the destination was carrying before. Copying the slot verbatim is exact rather than approximate, because a slot holds a displacement from the pointer value beside it per document 05 section 5.2.2 and a copy moves that value across unchanged, so a slot correct beside the source word is correct beside the destination word with nothing inside it to re-encode.

The two ends are where the rule has to say something. A destination word the copy covers only part of has its slot cleared, since half a pointer is not a pointer and a slot left beside changed bytes describes something that is no longer there. A whole destination word whose source has no slot to give, which is a copy out of a local or out of storage the monitor does not watch, is cleared for the same reason: the honest record is that nothing is known about the pointer now in that word, and the refusal at the first access through it is the cost of the source having no aux rather than a wrong answer about the destination. A destination that is not an allocation this runtime laid out has no aux in front of it and there is nothing to write.

**What a byte-wise write does with the metadata.** Everything that is not a copy. `memset`, `strcpy`, `strcat` and a `read` into a buffer put bytes where words used to be, and they have no source of metadata to hand over, so the destination's has to go. The init plane records the range as written and the type plane records a character write, both of which make a correct program quieter. The aux is the one that goes the other way, and it is the one that was missing until tamnd/rucc#1307's audit: a slot left beside a word whose bytes have been replaced does not refuse a correct program, it permits an incorrect one. A slot holds a displacement from the pointer value beside it, so a word a `memset` filled and then read back as a pointer answers with a capability based at that value minus the old displacement and as wide as the old object. The wild address is inside its own bounds by construction, and the version is the old object's and is live while the old object is, so the bounds question and the lifetime question both pass and the dereference goes through. With the slot cleared the same read recovers nothing, the capability is bottom, and the access is refused where it should be. That is the only place in the model where stale metadata can permit rather than refuse, which is why every byte-wise writing clause clears the aux over every word it reaches, the two at the ends included.

**The clause is the unit, and it is checked as one.** Which planes a wrapper maintains is decided by the word its row was spelled with rather than by the function, so `memset` and `read` owe the same three things and `memcpy` owes different ones. That makes the invariant two halves. The first is that every row writing anything was spelled with one of the writing words, which is a property of the table and is checked by walking it. The second is that each writing word really does maintain each plane, which is not a property of the table at all: the wiring lives inside the generator, where no walk can see it, so it is checked by running one real row per word over instrumented memory and looking at the three planes afterwards. Adding an eighth word to the grammar does not compile until somebody says there what it owes. Three holes of exactly this shape had been found by hand before the check existed, which is the argument for having it.

**What a write owes the epoch plane.** A wrapper that puts a pointer into a structure another thread can reach has stored to a pointer word, and the plane has to say who did it or the reader over there has nothing to compare itself against. So a destination word whose capability a copy changed is stamped, and a word that really did hold a pointer before a byte-wise write is stamped, on the word and on the aux slot beside it with one stamp, because the torn store question compares the two and stamping only the word would turn a correct program's next load into a report. The unit is a word whose slot the call changed and not every granule the call covered. That is not an optimisation, it is the same rule generated code follows: a race check is emitted only where the compiler knows the access is pointer shaped, the plane's granule is eight bytes because a pointer is eight bytes, and a granule two threads can both reach without racing is a granule holding no pointer. Stamping everything a `memset` covers would put two threads writing neighbouring members of one structure into the plane as each other's strangers. It also means the cost is nothing on the buffers these calls are mostly about, since the slot walk is already happening for the aux and a buffer with no pointers in it produces no stamps and does not even take one.
...
**`realloc` is a copy.** A resize is two instances by definition, so the shipped allocator never grows a block in place: growing one in place would put the same version over granules that were somebody else's a moment ago, and shrinking one in place would leave a live capability naming bytes the program has given back. Being a copy means it carries what section 10.3 says a copy carries, all of it, and the aux is the half that is easy to forget. An array of structures a program grows as it fills is the ordinary place for pointers to be among the bytes being moved, and a `realloc` that moved the words and left the aux would describe those pointers with whatever the block it landed in was carrying. The allocator uses the same function the interposed copy does, because it is the same copy with a different caller.
