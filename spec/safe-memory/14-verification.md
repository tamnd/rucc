...
| A real library survives the monitor and gets the right answers | instrumented SQLite, zlib, Lua, brotli and zstd, per commit | 14.9 |
...
**Cost.** Doubles the corpus run time, which was the argument for making it nightly rather than per-commit. Measured on the suite as it stands the doubling is nineteen seconds against nine, and it runs on every commit, because it is the only thing in this project that runs an instrumented program through the optimizer at all and leaving it out means the whole back half of the compiler has no execution coverage in the check people run before pushing. tamnd/rucc#818 is what that cost: a divide by zero in loop splitting, on a case that was already in this suite, caught by nothing until somebody compiled SQLite by hand. Its value was never in doubt anyway, since this is the mechanism that lets check elimination be aggressive, and without it the correct engineering posture would be to eliminate almost nothing. If the corpus grows to where the doubling is measured in minutes rather than seconds the question is worth reopening, and the answer then is a subset per commit rather than none.
...
## 14.9 Real libraries, end to end
...
**What the check is.** A table of projects. Each row names the sources, what the project's own configure script would define on a Linux machine, and every report the monitor is expected to make. Each row is compiled by this compiler at `-O0` and at `-O2` with the monitor on, linked against the runtime, and run against a workload whose answers are arithmetic. The program checks its own answers and the check reads its verdict.

**What the expected reports are for.** The workloads do nothing wrong, so a report nothing in the row expects is a false positive by construction and is a failure. A report the row does expect is a finding: the library really does the thing, the row says which site and why, and the check fails if it stops arriving, because that would mean either the library changed or a check stopped looking. The runs are made in the deduplicating posture rather than the aborting one, so one site says its piece once and the program still reaches the end and gets its answers right.

**The rows.** SQLite is one enormous translation unit that recycles small blocks through a lookaside allocator, and its workload inserts four thousand rows, builds an index over them, takes a count and a sum over a text pattern and reads fifty rows back in index order. It expects no reports. zlib is fifteen small translation units that take a few large buffers at the start and then index them with pointers held inside the caller's own structure, and its workload round trips four hundred kilobytes through the whole-buffer interface at three levels, through the streaming interface a thousand bytes at a time with an output buffer too small to hold one call's worth, and through the gzip file interface. It expects one report: `slide_hash` in `deflate.c` reads every two byte entry of `s->prev` before anything has written it, which zlib's own comment beside the loop acknowledges and considers harmless because the value is never consulted. Lua is thirty two translation units and is neither of those shapes: it is a language runtime, so every value it moves is a tagged union whose payload is sometimes a pointer and sometimes an integer of the same width, its collector reaches every live object by walking those unions, and every error it raises leaves through `longjmp` from the middle of a C stack the interpreter built. Its workload runs a script that grows two thousand tables past every rehash the implementation does, takes four hundred long strings apart with the pattern matcher, suspends and resumes fifty coroutine stacks forty times each, dispatches six thousand operations through metatables and raises fifteen hundred errors, half of them from inside the library rather than from the script, with a full collection between each phase, and then drives the C stack by hand through `lua_next` and the registry. It expects no reports. brotli is thirty one translation units doing zlib's job a different way: it carries a static dictionary of a hundred and twenty thousand words, splits its input into blocks and picks a different set of Huffman tables for each, keeps its decoder state in a ring buffer it grows as it learns how much it needs, and reaches back into that ring with a distance that is allowed to select the dictionary instead, which makes a distance a number that picks storage out of one of several places rather than a pointer to it. Its workload builds a buffer of three kinds of content, words the static dictionary knows, words it does not and bytes that do not compress, and round trips it through the whole-buffer interface at three qualities and through the streaming interface seven hundred bytes at a time with an output buffer too small to hold one call's worth. It expects no reports. zstd is thirty translation units and is the third compressor, which earns its place on two things neither of the others does. Its match finders keep several tables of positions over the same buffer at once and switch between them by strategy, so one compression indexes one piece of storage by three different schemes, and its dictionary builder runs a suffix array over the samples, which is the only code in the table that sorts pointers into a buffer rather than walking them, and a suffix array is a hundred thousand comparisons between two addresses into the same object. Its workload round trips two hundred kilobytes through the whole-buffer interface at three levels that are three different match finders rather than three settings of one, through the streaming interface eight hundred bytes at a time with an output buffer too small to hold one call's worth, and then trains a dictionary from three hundred small samples and uses it on a sample the training never saw. It is the first row whose reports are not held to a list, and the row says why: zstd keeps every position as a 32 bit index and one pointer that turns an index into an address, `ZSTD_window_update` computes that pointer as the caller's buffer moved back by everything the compressor has seen, and every match finder reads through it at an offset that lands back inside the buffer it came from. Section 4's J2 refuses the derivation and clears the capability, so 997 derivations turn into 4,710,697 refused reads at 39 distinct sites, with every answer still right, and that is document 17's question 12 with a library behind it rather than a test case. A list of 39 entries would say nothing and would break on the next release, so the row names the question instead, the check prints it beside the row, and it goes back to a list when the question is answered. The five libraries are five different shapes and none of them reaches the others' paths, which is the argument for a table rather than for one more anecdote.
...
**What it is not.** A handful of libraries and one workload each, so it is evidence and not coverage. Section 14.7's replay is the coverage claim and document 12's corpus is the number that means something. This is the floor under both: a monitor that cannot run SQLite correctly has nothing to say about a kernel.
