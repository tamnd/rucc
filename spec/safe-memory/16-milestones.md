...
The `cap` value type, the 19 instructions of document 06.2.2, the four facts, the plane metadata node kind. Verifier rules for every one. Printer and parser round-trip. `rucc-safety` exists as a crate at rank 10 and does nothing but insert checks on a hand-written test IR.
...
Both measurements are done. The first is in [document 05.2.5](05-representation.md) with its consequence written into question 6. It needed only the front end, so `--emit=type-granules` is a compiler flag rather than a script over DWARF, and the answer is that the compression works but the granule has to be 8 bytes and not the 16 document 05 assumed. That was the cheaper of the two orderings: had the type plane been built first at 16, the whole of it would have been rewritten at S5.

The second is in [document 05.2.6](05-representation.md), from `cargo xtask aux`, and it went against document 05. Adjacent aux does not pay for itself: an aux array is twice the payload, so adjacency buys the same page and never the same line, and the three to four times footprint costs more page walks than the shared page saves on six of the seven patterns tried. Document 10.4's adopted third-party allocators are acceptable, which is what this box was asked to decide, and whether the default layout should change with it is now question 11. It was measured by simulation because the monitor does not exist yet, and that is the point of running it here rather than at S5, when S1's allocator and S2's boundary would already have been built around the constraint.
