...
One thing is left open, and it is not a foreign write. A `memcpy` into an instrumented object leaves the destination's aux saying whatever it said before, and the runtime interposes that call and knows both ranges, so the answer there is to copy the aux across rather than to mark the destination handed. That waits on the same thing every other aux write waits on, which is generated code that emits a `cap_store`, and it is tamnd/rucc#1148.
