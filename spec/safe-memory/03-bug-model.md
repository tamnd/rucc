...
| S5 | Pointer arithmetic escaping its object | none | result outside `[lo - stride, hi]` of its provenance, one element either side permitted | bounds | ✓ | ✓ | ✓ |
...
S5's permitted window is `[lo - stride, hi]`, and the `lo - stride` end of it is a deliberate departure from C. C 6.5.6p8 permits a derivation to land anywhere from the first element to one past the last and nowhere else, so `&a[-1]` is undefined and `&a[n]` is not. The asymmetry exists for segmented architectures where forming the low address could trap, and no target this compiler supports is one of them. Meanwhile the address one before the first element is how a large amount of ordinary C is written: `p = &a[-1]` followed by a loop that pre-increments, which is how SQLite's bytecode interpreter walks its program, and `for (p = &a[n-1]; p >= a; p--)`, whose final decrement computes it. Both were written by people who knew what they were doing and neither reads or writes anything outside the object.

So the model permits one element before the first, exactly as it permits one past the last, and `stride` is the size of the type the arithmetic steps over rather than a byte. A rule counted in bytes would be useless, since `&aOp[-1]` on a 24 byte element is 24 bytes below the object, and permitting an arbitrary distance below would give the class away entirely. `&a[-5]` is not an idiom, it is a loop that ran too far, and it is refused. For arithmetic on a character type the stride is one and the low end of the window is one byte, which is where the strict reading already was.

What this costs is stated rather than hidden: S5 no longer reports a derivation that lands exactly one element before the first and is never accessed. Any access through such a pointer is still refused by J1, because the window a derivation may land in has always been wider than the window an access may touch, and only the derivation's window moved. The loss is therefore confined to pointers that are computed, never read through, and were wrong by exactly one element, and it is the same loss that has always been accepted at the other end of the object. This is the only row in this document where the model is more permissive than C, and it is here rather than in document 17 because it is a decision and not a question.

The first full pass over a corpus member put the sentence above about catching a derivation where it happens under some pressure, and the pressure is worth recording here rather than only in document 18. SQLite's `veryquick.test` at Tier D leaves five refusals and four of them are S5 on a pointer that is never read through: two loop guards that form `p + k` in order to test whether `p + k` is allowed, one pointer in SQLite's own test harness that is null when the arithmetic runs, and one in `fillInCell` that advances its source pointer on every iteration after the payload has turned into a `zeroblob` and the copy has become a `memset`. All four are undefined by 6.5.6, none of them touches a byte outside any object, and none of them is within one element of anything, so the window above does not reach them and widening it further would give the class away. Whether S5 should report at the derivation or hand back a pointer that cannot be dereferenced and let J1 report at the access is question 12, and it is a question rather than a decision because it changes what judgement J2 means in document 04.
...
This section is the one that decides whether the project survives contact with real code. Every entry is a widespread C idiom that a naive implementation of the model in document 04 rejects, and all but two of them are also legitimate C. The exceptions are the one-before-the-start row and the address-computed-to-be-tested row, both of which are undefined by the letter of the standard, are written by careful people on every target this compiler supports, and read and write nothing outside the object they came from. The mitigation column is the specification; anything not resolvable here becomes a document 17 entry.
...
| One-before-the-start pointers, `&a[-1]`, as the base of a loop that pre-increments or the last step of one that decrements | address == `lo - stride`, and C permits a derivation at the high end of an object and not at the low end | permitted by S5's window, which is `[lo - stride, hi]` rather than `[lo, hi]`. Only dereference is rejected, so nothing the program reads or writes moves. This is the one row where the idiom is not legitimate C, and section 3.1 says why the model deviates and what the deviation costs. |
| An address computed in order to decide whether that address is allowed, and a pointer a loop keeps advancing after it has stopped reading through it | the derivation leaves the object, so S5 refuses at the arithmetic and there is never an access to refuse | undecided, and question 12 in document 17. The four instances found so far overshoot by six bytes, thirty bytes, a whole address space and several kilobytes, so the window in section 3.1 does not reach any of them and cannot be stretched to. The candidate answer is CHERI's: permit the derivation, hand back a pointer that carries no capability, and let J1 report at the access that never comes. What that gives up is the sentence under section 3.1's table, which is the reason S5 is a class of its own. |
...
This table has been checked against one real program. [Document 18](18-sqlite-idioms.md) walks it row by row against SQLite 3.45.1: nine rows are exercised, five are not exercised by that program at all, one does not apply to C, and no row's resolution had to change. The audit did find three idioms with no row here, which are now questions 8, 9 and 10 in document 17, and they are questions rather than rows because none of them is a false positive under the model as written. Building and running SQLite at Tier D later found a fourth, which is the one-before-the-start row above, and that one is a row rather than a question because it *is* a false positive and a false positive at Tier D has to be decided rather than filed. Running the whole suite rather than the first two thousand tests found a fifth, the address-computed-to-be-tested row, which is both: a row, because it is a false positive and the rule says so, and a question, because the shape of the fix is a change to what a judgement means and not a widened window. Every later corpus member gets the same treatment, and a row that no corpus member exercises is a row whose resolution is untested however many members pass.
