...
This section is the one that decides whether the project survives contact with real code. Every entry is a widespread C idiom that a naive implementation of the model in document 04 rejects, and all but one of them are also legitimate C. The exception is the one-before-the-start row, which is undefined by the letter of the standard, is written by careful people on every target this compiler supports, and reads and writes nothing outside the object it came from. The mitigation column is the specification; anything not resolvable here becomes a document 17 entry.
