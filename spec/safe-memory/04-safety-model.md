...
C4 is the one of those four that is reported under J1 rather than under this number, and that is not an inconsistency. A use after free violates J1 whether or not a race produced it, the operation is refused on J1's account, and the race is the explanation rather than the violation. So the witness is attached to that report instead of raising a second one, and the pointer word check leaves storage nobody owns alone so that the pair really is one report. Which number a class is refused under is a question about what the operation did, and a class can be evidence for one judgement while being a race.
