...
**A pointer to the element before the first, brought back by the loop that reads it.** This one was found by running rather than by reading, which is why it is phrased differently from the three below it. `sqlite3VdbeExec` sets its program counter to `&aOp[-1]` and the top of its loop does `pOp++` before anything touches it, so the out of range address exists for the length of one increment and is never read through. The first full Tier D run of `veryquick.test` refused it at `alter-3.3.2`, after 2255 tests, and it was the first refusal of the run. It is a false positive under the rule in section 3.5, so section 3.5 now has a row for it and section 3.1 widened S5's window at the low end to `lo - stride` to hold it. That widening is the only place in the bug model where the monitor permits something C does not, and what it costs is written where the decision is. The reverse walk `for (p = &a[n-1]; p >= a; p--)` has the same shape and is covered by the same rule, which is worth saying because the two idioms would otherwise look like separate exceptions.
...
It does not establish that SQLite runs clean under Tier D, and it does not establish that these are the only idioms in SQLite that matter, because the method was to look for known shapes rather than to read 255680 lines. It establishes that the fifteen rows written before anyone read a program survive the first program they were read against, that the three things they miss are findable by reading, and that the two rows that will hurt most on this corpus member are the two about allocators, both of which point at the same piece of unbuilt machinery in document 10.

Running it added the fourth entry in 18.5, and that is the useful part of the comparison. Reading found three idioms and all three turned out to be questions about a check that is off by default or about a cost rather than a correctness problem. Running found one idiom and it was a false positive on the first refusal of the first full pass, which is what reading does not do and is the whole argument for doing both.

The next corpus member should be audited the same way and the differences recorded, because the value of this document is not in the sixteen verdicts, it is in whether the seventeenth row keeps being needed.
