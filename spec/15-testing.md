...
"Will not be discharged" means two different things and the gate promises the weaker one. A solver can be unable to settle a claim at all, which is a fact about the claim, and it can run out of the budget it was given, which is a fact about the budget and the machine. The gate treats both as the same shrug, deliberately: a solver that stops has no way to tell you which of the two it did, so a gate that promised the stronger reading would be promising something it cannot check. What follows is that a bounded proof is a fallback from a shrug of either kind, and that the budget is part of what a run means rather than a detail of how it was configured. That is why the number is printed on every run and why it is set from the measured cost of the slowest rule in the tree with a wide margin over it rather than from anything anybody would call a timeout. A budget tight enough to be reached by a rule that is fine turns that rule into a rule nobody has proved, and the same tree then proves and fails to prove the same rule minutes apart.

The tests over the gate itself are held to the stronger reading, because a test about what happens after a shrug should not be arranging the shrug with a clock. They ask through a small trait rather than the solver directly, and the one question no solver settles is answered by a stub that says so. Everything narrower goes to the real solver, so what is stubbed is the part the test is not about.
