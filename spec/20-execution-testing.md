...
The file is the whole rule set rather than the part of it that fired: one line per rule in the order the rule file writes them, each saying `fired` or `unused` and then the file, the line and the pattern, under a first line holding the count. A single one of them therefore says what there was to cover as well as what was covered, and unioning a directory of them is a merge of marks rather than a merge of one list against a rule file the reader would otherwise have to parse for itself. One file is written per run of the compiler rather than per input, because what a run reached is one answer however many files it was asked to compile.
