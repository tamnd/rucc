...
An atomic type that does not fit in one is refused where it is written, rather than being accessed without a lock. GCC calls libatomic for one of those, which takes a lock out of a table keyed by the address, and a program half of whose accesses take that lock and half of which do not is not atomic at all. There is no libatomic here, so the choice is between refusing the declaration and quietly compiling a program whose atomics are not atomic, and refusing is the one a user can act on. When a lock table ships, this becomes a call and the refusal goes away.

Half of that has now shipped and the refusal has not moved yet. `runtime/builtins/atomic.c` is the lock table and the four routines that take it out of it, which `spec/12-abi-and-runtime.md` section 12.8 describes, and what is missing is the pass that turns a wide access into a call to one of them. Until that lands the refusal is still the honest answer, because the routines existing is not the same as the compiler reaching them, and a declaration that was accepted and then accessed without a lock would be the outcome this paragraph exists to avoid.
