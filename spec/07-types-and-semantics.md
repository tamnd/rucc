...
The `__atomic_*` builtins are the other half and are not there yet. One of them on an object too wide for an instruction is refused by name, so a program gets a message about what it wrote rather than one about an instruction the back end has no rule for. The operators are what this section describes and they work at every width.
