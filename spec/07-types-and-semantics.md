...
Until the table shipped, a type that did not fit was refused where it was written, which was the honest answer at the time and is recorded here because a program that used to be turned down is now compiled. What is still refused is narrower: an object the walk has no single value for, which is a structure, a union, a vector and a complex type. The runtime would take the lock for one of those quite happily, since its routines work through a pointer and a size, but an access to one is a copy rather than a read and there is nowhere for the walk to put what came back. That is the next piece of the work rather than a rule.
