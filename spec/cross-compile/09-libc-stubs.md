...
Build systems pass `-lm` and `-lpthread` unconditionally and a missing file is a link error, so a sysroot has to contain a file for every name a link line might ask for even when there is nothing in it. We generate those empty libraries. Which names they are is per libc, and the two libcs differ in a way that is easy to state wrongly.

On glibc 2.34 and later, `libpthread`, `libdl`, `libutil` and `libanl` are merged into `libc.so.6` and the separate shared objects are kept as empty compatibility stubs. Those four are the generated set. `libm` is **not** in it: `sin` is in `libm.so.6` on every glibc there has ever been, so an empty one would be a link failure for any program that does arithmetic. `librt` is not in it either, for a different reason, which is that it has been moving into `libc` across several releases and so whether it is empty depends on the version. Both files do have to exist in a glibc sysroot and both come from their `abilist` description like every other real library. `libcrypt` left glibc for libxcrypt rather than merging into it, so on a modern distribution it is somebody else's library sitting in the same directory.

On musl none of them ever existed separately and the shape of the answer is different as well as the list. musl ships empty **archives**, its `EMPTY_LIB_NAMES`, which is `m`, `rt`, `pthread`, `crypt`, `util`, `xnet`, `resolv` and `dl`. `libm` is in that list, which is the row that makes the difference concrete: the same `-lm` finds real code on glibc and nothing at all on musl. The archive form matters and is not a detail of packaging. A link naming an empty archive records no dependency, which is what musl wants because there is no such file on a musl system; a link naming an empty shared object records a `DT_NEEDED` on its `SONAME`, which is what glibc wants because the file is there on the target. Getting the two the wrong way round gives either a program asking the loader for a file that has never existed, or a program missing a dependency it needs.

One more thing that is easy to get backwards: the name the linker opens is not the name that ends up in `DT_NEEDED`. `-lpthread` makes the linker look for `libpthread.so` and what goes into the program is the `SONAME` inside it, `libpthread.so.0`. A distribution has both and one is a link to the other. A generated sysroot needs the one the linker opens carrying the `SONAME` of the one the loader wants, and writing the file under the `SONAME` leaves `-lpthread` with nothing to find.
