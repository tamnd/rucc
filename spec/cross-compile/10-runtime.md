...
**How it is built.** `rucc-builtins` is C compiled by rucc itself for each target, per parent document 12, which said Rust until tamnd/rucc#912 settled the two documents against each other and section 12.8 records why the answer is this one. That is a bootstrapping property worth keeping: it means every target's runtime is evidence that the target's codegen works, and a target whose builtins do not build is a target that is not ready. The archives are built at rucc's release time for every tier-1 and tier-2 target and shipped, because building them on demand puts a compile in the middle of every user's first link. `cargo xtask builtins --target <triple>` is that build: it compiles the compiler, then compiles every `.c` in `runtime/builtins` with it in one command line and writes the archive straight out, which is what `--emit=archive` is for. Today that is the four block routines, the 128-bit division and modulo, the same division at sixty four bits, and the one target whose objects rucc can write, and the other targets fail the task by name rather than silently producing nothing, which is the property in the paragraph above doing its job on the first day it could. The division is the row above marked every 64-bit target, and it is the first row of this table whose C needed the backend to do something the block routines did not: a 128-bit compare and a variable shift in a loop, on parameters that arrive in four registers. Section 12.8 of the parent document records what it is written as and what holds it against a reference, and the short version is that the shipped C goes a bit at a time and the reference goes in base 2^32, so the two can only agree by being right. It is also the first row whose caller had to be written: a `/` on a value this wide was refused by the back end until the routines existed, and what emits the call is the pass that splits the width, held against gcc by running it in `cargo xtask wide`. The conversions row is the third piece in the archive: `runtime/builtins/convert.c` is the eight names a cast between a 128-bit integer and a `float` or a `double` becomes, which is the part of that row a 64-bit machine needs, and section 12.8 records the two algorithms and what the undefined cases do. The compiler emits those calls out of the same pass that splits the width, so a cast at this width compiles and runs, and `cargo xtask wide` holds all eight against gcc at three optimization levels. The rest of that row is the eighty bit float, which the back end has no register for and is tamnd/rucc#326, and binary128, whose arithmetic is in now, so what the quad spellings of these eight wait on is the writing of them and nothing else. The first row of the table is in the archive now as well, which is those same six names one width down: `__udivdi3` and its five relatives, what a `/` or a `%` on a `long long` becomes where a register is thirty two bits. It went in ahead of any back end that emits it because it is the same loop and the same reference with the widths changed, and that is the order the 128-bit set went in at too, so the evidence for it is the differential in section 12.8 rather than a program that ran. The consequence is the one worth having: the first 32-bit target to reach a division finds the routine waiting instead of a refusal. The soft float row is the fifth piece and the first row that is only partly in: `runtime/builtins/float.c` is the four operations and the negation on a `float`, which is `__addsf3` and its four relatives, and the wider formats came after it one at a time. Section 12.8 of the parent document records the two shapes, three rounding bits in the shipped C against exact integers and a single rounding in the reference, and the reason the formats go in one at a time rather than together, which is that the mistakes in a routine this size are in its shape and the shape is the same one width up. It also records where the evidence ends, since the reference is held against this machine's own floating point unit as well as against the C, so the chain reaches hardware rather than stopping at two implementations agreeing. The eight comparisons are in there too, which are what `a < b` on two floats becomes on the same target, and so are the eight conversions to and from an integer, which is what a cast becomes there and which section 12.8 records as a rounding on the way up rather than the widening people expect. Single precision is complete, the pair that widens to a `double` and narrows back included, which waited on double precision existing and is described at the end of this row. Double precision has started, since `runtime/builtins/double.c` is the four operations and the negation on a `double`, and it is the row's second format rather than a rewrite of the first: the shape is the same one and section 12.8 records the two routines where it is not, a product that no longer fits in a word and a division that the reference cannot write as a `/`. The eight comparisons at that width are in too, which are the same eight shapes over one comparison and took a short paragraph in section 12.8 rather than a long one, because a format written second is mostly a format already written. So are the eight conversions to and from an integer, where the one thing that differs from the narrower set is not a decision anybody made: fifty three bits hold every `int` exactly, so the two narrow conversions going up never round at this width, and coming down the bound that matters is 2^63, which is a double while the largest `long long` is not. The pair between the two formats is in as well, which is `__extendsfdf2` and `__truncdfsf2` and is the first thing in this row that needed both formats to exist, so it went last. Both sides keep it with the narrow format, because a widening needs nothing of the wider format but where its fields are and a narrowing is a rounding into the narrow one. Binary128 has started, and it is the one format in this row that is not about a target without a floating point unit: `runtime/builtins/quad.c` is the four operations, the negation and the eight comparisons on a `_Float128`, and no machine has an instruction at that format, so those thirteen are a call on every row of the matrix rather than only on the rows with no unit. Section 12.8 records the three things that differ from the format below it, which are a significand that no longer fits in a word, a reference written in a language with no type for the format, and two answers about a not a number that libgcc decides per architecture while one archive here cannot. It also records what was found by comparing the shipped C against libgcc's own routines over ninety three million operands, which is that a subtraction must hand back the not a number that came in rather than one with its sign flipped, and that the two formats below had been getting that wrong. The eight conversions between a quad and an integer are in too, and they are the one place in this row where the wider format is less work than the narrower ones: a hundred and thirteen significant bits hold every integer a sixty four bit machine has, so nothing rounds going up, and coming down the largest exponent with an answer is sixty three, so the significand only ever moves down. Section 12.8 records the fourth oracle they needed, which is a table of libgcc's answers split by value band because libgcc saturates a conversion with no answer where this library answers zero, and the pair of mutations where each family of the differential catches what the other one misses. What is left of the row is the four conversions between a quad and the two narrower formats, and the lowering pass that turns an operation on a `_Float128` into a call to any of this, without which the routines are in the archive and no C program reaches them.
