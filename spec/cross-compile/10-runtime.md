...
**Where it is looked for, and what a link does without it.** Three places, in the order a `-B` prefix has everywhere else: what the user named with `-B`, then the sysroot's `lib` beside the libc, then beside the compiler, which is the build tree. A cross link that finds none of the three is written without the archive rather than naming a file that is not there, and that is a deliberate choice rather than a shortcut. Nothing builds the archive for a target yet, so today it is the usual case, and a program that reaches none of those routines links and runs. One that reaches one of them gets the linker's message naming the routine, which is the more useful of the two messages available: the other one says a file is missing and leaves the reader to work out which routine wanted it.
