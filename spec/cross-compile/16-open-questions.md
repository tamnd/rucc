...
**What is now measured.** The size of the obligation, which was the part of this question that could be answered without waiting for a release. `bin/glibc-versions` in `tamnd/rucc-cross` says that 210 of the 498 headers in 2.28 through 2.44 change their code somewhere in that range and 288 never do, so the merge is a few hundred files and not five hundred, and a release that touches none of the 210 costs nothing at all. Six files change at every release and are the ones to look at first. That does not answer who does the work, but it sets the scale the next glibc release will be measured against, and it gives the validation half of the question something mechanical to stand on: the record is regenerated and compared per night, so a release that changes a file nobody expected it to change is visible before anybody edits the tree.
