...
**Three disagreements with the reference, on the first run.** Compiled with the pinned Zig 0.16 in tamnd/rucc-cross, eight of the fifteen files compiled clean and seven did not. Per section 6.9 the reference is the ABI in all three cases, and all three are the same underlying shape: rucc has one record layout algorithm and these targets need three.
...
**MSVC gives an empty struct a size of four, and mingw gives it zero.** So does a struct whose only member is a zero width bit-field. The alignment stays one in both cases, which is the part that surprises: it is a four byte object with a one byte alignment. The number is what clang's Microsoft record layout produces and it is worth confirming against MSVC itself before the fix is written, because everybody's first guess is one and everybody's first guess is wrong. rucc says zero.

None of the three is a bug in `layout_record`'s arithmetic. All three are facts about a target that `TargetInfo` does not carry, which is why the corpus had to exist before the fix could be written: the diff of the fix is now readable as exactly the set of numbers that changed.
