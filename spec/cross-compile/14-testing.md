...
**The consequence is a policy, not a caveat.** Every target's support statement records whether its evidence is hardware or emulation, and a **known-qemu-divergence list** is maintained per target, every case where a test was skipped, adjusted or expected to differ under emulation, with the reason. That list is the honest measure of how much the emulated evidence is worth, and it is published. A target whose divergence list is long is a target whose tier claim should be re-read. It is `docs/DIVERGENCE.md`, `docs/TARGETS.md` points at it, and it lists the rows with no execution evidence as well as the ones with divergences, because absence from a list like this reads as a clean bill of health and for most rows today it is not one.
...
Fifty eight of them take a fixed argument list, ten written by hand and forty eight drawn from a fixed seed. The hand written ten are the cases somebody named: ten integers, ten doubles and twelve of them alternating, all past the end of every argument register file here, the aggregates that fit in registers, the homogeneous floating point ones that AAPCS64 puts in vector registers and SysV packs into SSE eightbytes, the twenty four byte one that is past every threshold, a large aggregate returned through the hidden pointer that moves every other argument along, a function returning `void` with the registers full, a `long double` between two integers, and the nested struct and the union that a classification rule has to flatten before it can decide anything. The drawn forty eight are for the orders nobody thinks to write down.
...
`__int128` is out for now, for the same reason it is out of the seeded half of the record corpus: three rows of the table do not have the type, and one source that serves every target is worth more than a corpus that needs a preprocessor conditional to say which types exist.
...
## 14.11 The variadic half, and the bug it found on the first compile

Thirty four of the ninety two functions take a `...`, ten written by hand and twenty four drawn from the same seed as the fixed ones. They are appended after the fixed groups rather than mixed in, so that the day they were added the diff was only the new material and the fifty eight signatures that had already passed four builds did not move.

The `...` is where the ABIs stop agreeing, which is the whole reason for the group. Document 06.2 item 6 has five answers across the table and only one of them makes a variadic argument travel differently from a fixed one: Darwin arm64 puts every variadic argument in memory, whatever it is and however empty the registers are. x86-64 System V passes them in both banks exactly as it passes fixed ones and needs `al` set to the number of vector registers used, and the Windows and AAPCS64 answers are the same as their fixed ones. So the group is a real question on one target, a check that nothing was broken on the rest, and a place for the next back end to fail loudly.

**The generator has to respect two separate rules about `...` and it walked into the second one.** The first is the default argument promotions of C17 6.5.2.2p6: a narrow integer becomes an `int` and a `float` becomes a `double` before it travels, so the callee reads back a type the caller never wrote and the corpus has to store the promoted type or check the wrong thing. That one was written in from the start. The second is C17 7.16.1.4p4: the parameter that `va_start` names must not be one the promotions would change, and the standard makes that undefined behaviour rather than a diagnosable error. Clang diagnoses it anyway as `-Wvarargs`, which is how the first generated corpus failed to compile at all. The fix promotes the last named parameter in place rather than redrawing it, so the seed spends the same number of draws and adding the rule did not move the rest of the corpus.

**The lowering side is one enum.** `rucc_lower::abi::plan` walked the argument list asking `Call::argument` for every position, which is the right question for a fixed parameter and the wrong one past the `...`. It now carries a `Position` of `Return`, `Fixed` or `Variadic` and asks `Call::variadic_argument` for the last, where being past the parameter list only counts if the callee declared a `...` at all. A call with more arguments than parameters and no `...` is a program with no prototype in scope, and every one of those arguments is a fixed argument that nothing declared, not a variadic one.

**The first compile of the new corpus found a real gap in the back end.** rucc refused four functions with `no rule lowers a 'va_arg' producing a 'f80' [E0653]`. A `long double` on x86-64 System V is class X87, and X87 is the one class with no register in the save area, so a variadic one is always in the caller's argument area, aligned to sixteen and stepping sixteen. The walk in `crates/rucc-codegen/src/varargs.rs` was the general one, which compares an offset against the end of the register save area and branches, and there was nothing sensible for it to compare. It is now written out separately with the register half deleted: no compare, no branch, no new block, just the overflow pointer rounded up and read. That is what a differential corpus is for, and it found this before a single one of the four builds ran.
