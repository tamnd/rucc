...
Item 6 says which `librucc_builtins.a` and it took tamnd/rucc#1514 to say where it is. The line named it inside the sysroot, as the `lib` directory with the name joined onto it, and nothing ever put a copy there: it is this compiler's own output for the target rather than anything the platform ships, `cargo xtask builtins` writes it beside the compiler, and a sysroot fetched from a release holds what mingw-w64 or musl built and not what we built. So the path comes from the driver, which is what knows where the compiler is and what a `-B` prefix said, and the line takes it as a parameter. Two places that each named a file neither of them put there is how a Windows link ended up resolving the stack probe out of whichever `libgcc.a` was on the machine, which is the dependency publishing a sysroot exists to remove.

A link that wants it and cannot find it is refused by name rather than attempted. What the linker says instead is that `___chkstk_ms` is undefined and that mingw-w64's own objects are the callers, or that `__udivti3` is on a musl target, and neither of those is anybody's first guess at an archive that was never built. The message names `cargo xtask builtins --target=<tuple>`, which is what writes one, and names `-fno-builtins-lib` for somebody who meant to link without ours. It is asked in the preflight rather than after everything has compiled, and it is asked after the sysroot check rather than before it, because a machine with no sysroot at all should hear about the sysroot.

The construction of that line is `(tuple, sysroot, options) → argv`, a pure function with no environment reads, which makes it directly testable: a golden-file test per target comparing the generated argv against a recorded one. That test is cheap, catches regressions in the highest-consequence code in the driver, and needs no target machine. The runtime does not cost that property anything, because what the function takes is the path and what finds the path is the caller: the recorded lines write `<builtins>` where a machine would write a directory, for the same reason they write `<cache>` for the sysroot.
...
- **A path the toolchain made up rather than one the user named.** On COFF, clang writes a `.debug$S` section into every object whether or not debug information was asked for, and it holds an `S_OBJNAME` record naming the object file being written. `-g0` does not reach it and no `-g` flag turns it off, so it is set rather than suppressed, and the empty string is what clang itself writes when nothing asks for a name. That one record is the whole of why a produced windows-gnu sysroot differed between two hosts in 78 files out of 938, tamnd/rucc-cross#30, and the lesson generalises past COFF: a build directory that never appears on a command line still reaches the output if a tool invented a temporary path inside it.
...
One of those checks cannot be made by looking at a file, and it is the version. A windows-gnu link reads import libraries in which mingw-w64's `==` aliases, `_crt_atexit == atexit` among a few hundred more, are `IMPORT_NAME_EXPORTAS` records, and lld reads that encoding from 19 on. An older one does not refuse it and does not warn: it writes an import by ordinal zero, the link exits successfully, and the program dies at startup. Ubuntu 24.04 is the current LTS and ships lld 18, so this is an ordinary machine rather than an exotic one, and it is the shape this ordering exists to prevent with the fallback taken out of it. So the linker is asked what it is, once, and an lld older than 19 on a windows-gnu target is refused by version rather than tried. What it is not is a step back to the next candidate, because the next candidate is GNU `ld` under its mingw prefix and that one cannot read an import library llvm-dlltool wrote at all. tamnd/rucc#1515.

The check is narrow on purpose. Every other target is left alone, since this is one encoding in one format, and so is every linker that does not say it is an lld, since what the check can establish is that a specific old one is here rather than that anything else is fit. A linker that will not run or whose version does not parse goes through, and the link says what it says.
