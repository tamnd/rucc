...
**The relocation is counted from the other end.** Both formats write the distance from an instruction to something into four bytes and they disagree about where that distance is measured from. ELF counts from where the four bytes start and lets the addend make up whatever else was wanted, so one relocation type covers every instruction and the writer folds the rest in. COFF counts from where the instruction ends, which is not a number the record has a field for, so it is in the relocation type instead: `IMAGE_REL_AMD64_REL32` is an instruction that ends at the hole and `REL32_1` through `REL32_5` are one whose last one to five bytes come after it. The two numbers cannot be recovered from each other, since a displacement of minus four with nothing trailing and a displacement of zero with four bytes trailing are the same sum, so the relocation our assembler hands the writer carries both. That is the one change this format asks for outside the writer, and it is the reason the layout of a file is one piece of code for both formats with a short list of answers beside it rather than two writers.

**What is written and what is refused.** An x86-64 object is written: the text, the variables in `.data`, `.rdata` and `.bss`, the symbols with their storage classes, the relocations, and the names the file wanted and does not have. Anything the format has no way to say is refused by name rather than written as the nearest thing, which is thread-local storage, a reference through a global offset table, a record of where a patcher's room is, and a section the ELF startup code would gather. Each of those is already refused earlier for a Windows target, so the refusal in the writer is the backstop rather than the diagnostic.

**`.pdata`/`.xdata`.** Windows x64 and ARM64 exception handling is table-driven from a per-function record describing the prologue as unwind codes. As document 06.4 notes, only certain prologue shapes are expressible, so this format constrains code generation. Both sections are written for x86-64 and the linker merges them. `.pdata` is one fixed row per function of three image relative addresses, saying where the function starts, where it ends and where its description is, which needs a relocation this format has and ELF has none of. `.xdata` is the descriptions, each four bytes of header and then the codes, in decreasing order of where in the prologue they happened because the runtime reads them from the address it is unwinding at and works back to the front of the function. A row reaches its description by a local name of the form `$unwind$f`, which is what MSVC writes, because the two are in different sections and no distance between two sections is a number a compilation can work out. The codes come out of the same rows the DWARF table is built from, since what a row says is what one instruction of the prologue did to the frame and that is the question both tables answer, and the offsets are translated once: a row counts from the end of the frame and a code counts from where the stack pointer ends the prologue. A prologue the codes have no spelling for is refused by name rather than written as the nearest thing, which today is the frame pointer form and is what document 06.4 describes. On ARM64 there is additionally a packed unwind encoding for simple frames that is worth emitting because it is much smaller.
