...
1. **Fundamental type sizes, alignments and signedness.** `char`'s signedness (unsigned on AArch64 Linux, ARM, ppc64le, s390x; signed on x86). `long`'s width (`DataModel`). `long double`'s width *and format*, which are separate, 16 bytes of which 10 are x87 on x86-64 Linux, 8 bytes of `double` on Darwin and under MSVC, IEEE binary128 on AArch64/s390x/RISC-V, and IBM double-double on legacy ppc64. Windows is two answers rather than one, because mingw-w64 keeps GCC's x87 `long double` and only MSVC narrows it, so the rule reads the environment component and not the OS.
2. **Struct, union and array layout.** Usually derivable from member alignment, with exceptions that are not: System V i386's 4-byte `long long` alignment, which mingw on the same architecture does not share, s390x's 8-byte cap on scalar alignment, which makes a 16-byte `long double` 8-aligned there and 16-aligned on AArch64 with the same format, and s390x's high-order-first bitfield allocation.
