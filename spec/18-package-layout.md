...
│   ├── rucc-unpack/         0  zip, cabinet and compound file readers, and the deflate under them
...
The same requirement bounds what a file in this tree may be called. A path has to be one all three hosts can write, which is narrower than what Linux allows: Windows refuses a component whose stem is a reserved device name such as `AUX` or `CON` whatever extension follows it, refuses the characters `< > : " | ? *` and a backslash inside a component, silently strips a trailing dot or space, and holds two names that differ only in case to be one name. A path that breaks any of those does not fail a test on that host, it fails the clone, so nothing after the checkout runs and the failure is a line of git output with no compiler in it. `cargo xtask paths` is the check, it reads the list of tracked files, and it runs on the hosts that can check the tree out because the host that cannot is in no position to report anything.
