...
Negation is inside that boundary too, and it is folded. 754 says a negation flips the sign bit and copies every other bit, for every input including a NaN and a zero, so it is exact, it raises nothing and it does not consult the rounding mode, and none of the three questions the paragraph above is about arises. The reason to bother is that C has no negative floating constant: every one of them is a unary minus applied to a positive one, so `-1.0` reaches the middle end as an `fneg` of an `fconst`, and without the fold the back end makes a constant, a mask and three moves through a general register out of what should be one load. That is every negative floating literal in every program, at every level, and it is issue 1427. A bitcast of a constant is folded beside it for the same reason and on the same grounds, since it is the same bits read as another type of the same width, and what it unblocks is `fabs` and `copysign` of a constant, neither of which is a call: the front end lowers both to a mask over the bits, so without the fold the mask and the two bitcasts around it survive to the back end computing a number the compiler already has.
