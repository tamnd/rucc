...
gcc runs it in a loop, so when `a` and `b` are themselves the same conversion in each arm that is factored too, and the select moves down the chain. rucc does the same, down to `PHIOPT_FACTOR_DEPTH` levels, four. It matters because a level left in the arms is work the cost rule counts. `total += (long long)((x * 3) ^ mask)` against `total += (long long)((x + 7) ^ mask)` shares an add and a widening, and factoring only the add left three instructions in each arm, which is over the budget of two, so the branch stayed. With the widening factored too it is a select between the two masked values, and on the if-conversion facet in tamnd/rucc-corpus at an even rate it runs in about 130ms where the branch took about 215ms and gcc 16 takes about 170ms (#1904). Like gcc, only an operation of one operand is factored below the first level. An add there, as in `total + (i + i)` against `total + (i + 1)`, becomes `total + (i + select(i, 1))`, and after unrolling that is an add of a select where there had been a select of two constants, which the corpus's loop-unswitch shapes showed as 60 programs growing at O2. For the same reason a conversion is not factored below the first level when what each arm converts is a constant, since the two fold into two constants and a select of those is what the level above already writes. Unrolling also leaves selects between two constants that are the same number, one per arm, and constant folding turns those into the number.
