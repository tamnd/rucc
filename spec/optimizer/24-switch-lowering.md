...
**What is built today.** All four cluster shapes are written in `crates/rucc-codegen/src/switch.rs`. A stretch of at least eleven clusters at `-O2`, or six at `-Os`, whose values span no more than eight times the compares it replaces, or three times at `-Os`, is a jump table, unless it spans a word and goes to three places or fewer, where the bit test is kept as gcc keeps it. The two ratios are gcc's, and the two counts were measured on an AMD EPYC, where the walk beats the table below eleven arms when the value is hard to predict and the table is only smaller from six. The table is left in the IR as a range check and a `switch` over an index from zero, and the x86-64 lowering reads that as a `lea` of the table, a `movslq` of the cell, an add and an indirect jump. The table is four bytes a cell, each the distance from the table to its block. On x86-64 ELF it goes in `.rodata`, or `.rodata.` and the function's name under `-fdata-sections`, as gcc puts it, and each cell is a `R_X86_64_PC32` against the function's section. Everywhere else it is written after the function's last instruction, where it needs no relocation. tamnd/rucc#1548, tamnd/rucc#1759.
...
Where every answer is the address of read only data this file defines, or a constant number of bytes into it, the table holds how far each answer is from the table rather than the answers themselves. An array of addresses would need a relocation in every cell that the loader fills in, so in a position independent executable or a shared library it has to go in `.data.rel.ro` and costs a relocation per cell at every start. That is why gcc 16 only builds a table of addresses with `-fno-pic`, and rucc does not build without PIC at all. A distance is four bytes, the linker works it out once, and the table stays in `.rodata`, which is what clang calls a relative lookup table. Each cell is written as `.long name+k - .` with `k` the cell's own offset in the table plus the bytes into `name`, and the arm becomes the load of a cell, widened with its sign, added to the address of the table. The addition is done on integers and turned back into a pointer, because the pointer that comes out is into `name` and not into the table, and a `ptradd` from the table would tell alias analysis otherwise. A name qualifies only if it is constant, defined in this file and cannot be replaced at link time, which is the test `crates/rucc-opt/src/image.rs` already makes for the passes that read constant data, so a `static const` array or a string literal does and a `const` global another file could define does not. Only x86-64 ELF builds these, since that is the only target whose object writer knows a four byte distance to a symbol today. tamnd/rucc#1775.
