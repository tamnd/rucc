...
The widening is the part that has not been lifted. A chrec is extended only when its base and its step are both plain numbers, because widening `{start, +, 1}` in `int` wants `start` sign extended and that extension is an expression nothing computes. The form above has room for a second value but no room to say that a value in it is read through an extension, so a walk from an index the caller handed in is refused at the extension rather than at the address. That is why the room for two values is worth almost nothing on its own, one check on the SQLite amalgamation, and worth having as the thing the widening lands on top of.
