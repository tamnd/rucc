...
**What an invariant is, and why it has room for two values.** An invariant here is `on + scale * value + offset`, where `value` and `on` are values defined outside the loop and the two numbers are numbers. The linear part is what lets `j = 2 * i + 3` come out as `{3, +, 2}` rather than unknown, since the base and the step of that chrec are expressions nothing in the function computes. The `on` is there for one shape and one shape only: a pointer with an index the loop did not start at zero. `a[start + i]` has a first address of `a + 4 * start`, two values with a number for neither, and a form with room for one has to answer unknown to it. Nothing scales `on` and nothing negates it, because what it was added for is a pointer and a pointer is not something a loop multiplies. A sum that would need a third value is refused, which is where the subset ends and where the answer becomes unknown rather than wrong.

**The widening, and the value nothing computes.** Widening `{start, +, 1}` in `int` gives `{sext(start), +, 1}` in `long`, and `sext(start)` is an expression nothing in the function computes. A form that could only name values had to refuse the whole widening on that account, so a chrec was extended only when its base and its step were both plain numbers. In C the index of an array is an `int`, so that refusal is the first thing a walk from an index the caller handed in runs into, before anything about the address is even asked.

So the extension is described rather than named. A value in an invariant carries how it is read, sign extended or zero extended, and at what width, and whoever builds code from the invariant emits the extension. Each end of the chrec has to be a plain number or one of a value and nothing else, because the narrow arithmetic may already have wrapped and `sext(2 * x + 3)` is not `2 * sext(x) + 3`. An extension of an extension collapses into one wherever the two mean the same thing, which is everywhere except a sign extension read as unsigned afterwards.

The two together, room for a second value and a widening that has a form, are what let `a[start + i]` be followed. On the SQLite amalgamation at `-O2` with detection on they take 158 checks and 12 sites off the largest row of loop splitting's census, the checks it keeps because it cannot say what their address does round the loop, and 12 more loops get split.
