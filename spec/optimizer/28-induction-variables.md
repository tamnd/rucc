...
Two places where nothing happened are worth recording. The SQLite amalgamation went from 243607 instructions to 243606, one, because `candidates` only makes a countdown for a loop whose trip count is a number and almost every loop in SQLite counts to something it works out. Widening that to a count the loop works out in its preheader was the obvious next thing and is below. And `-Os` is unchanged on every one of the 2663 sources, because ivopts is not in the pipeline at `-Os` at all.
...
**A count the loop works out.** tamnd/rucc#1696 is the widening the paragraph before last asked for, and it is `Start::Worked` beside `Start::Fixed`. Where the count is an expression, the variable starts at that expression clamped at zero, plus one, worked out in the preheader in sixty four bits whatever the counter is. The clamp is the same `crate::loop_delete::clamped` a walk's reach uses, and it is there for the same reason, which is `Entered`: `for (int i = 0; i < n; i++)` with a negative `n` is a loop that goes round no times, the count the analysis gives for it is negative, and a variable that starts at one refuses the first time it is asked. So `candidates` asks `comes_back` rather than `under_undefined_overflow`, since the one assumption between the two is the one the clamp pays for.

The clamp is a compare and a select in front of every loop that gets a countdown, and most of the time it is not needed. Header copying puts a test in front of the loop that says it goes round at least once, and where the ranges at the preheader say the count cannot be negative, `settled` drops the clamp and writes the plus one straight onto the count, folding it into a constant already there. Without that a countdown trades a compare in the loop for a compare and a select in front of it, which only pays where the loop goes round more than once.

Getting the countdown to fire on real loops took three fixes outside this pass. The first is in `crate::scev`. A loop that counts down to zero with `!=` has a count whichever side of zero it starts on, because it can only get to zero from above, so it no longer carries `Approaching` and a count that carries it is one `loop-delete` refuses. A countdown to anything else still carries it, because a start below the limit really does go round until the counter wraps. The second is also in `crate::scev`: a test asked after the step has a start of `start - 1`, which the unsigned reading refused as a base, and it now takes it where the step cannot wrap. The third is in `crate::loop_delete`, which now sweeps up after itself: a loop it deletes leaves the arithmetic the exit value was worked out from, and a clamp nothing reads was left standing in front of the empty loop.

One refusal came out of the measurement rather than the design. `collect` only sees the uses inside the loop, so a counter read after the loop, or by an inner loop's bound, looked unused, and the countdown went in beside a counter that stayed live. The inner loop case cost a spill. `read_elsewhere` follows the counter round its cycle and refuses the countdown when anything outside the cycle reads a value in it, which also stops the exact count case in tamnd/rucc#1700 putting a countdown on an outer loop whose counter an inner one needs.

The numbers. Over the 2663 corpus sources at `-O2`, measured as assembly text, 183704 instructions became 183494, 210 fewer. Forty six sources differ: 44 are smaller and 2 are larger, `long-double.through-varargs` by 4 and `induction-variable.starts-past-the-limit` by 2, both a register move the allocator put in. `-O1` and `-Os` do not move, since ivopts is in neither. The SQLite amalgamation went from 243606 to 243511, and the pass now writes a countdown in 6 of its functions rather than 1. The other 30 loops in it that could have one are refused because something in the body still reads the counter, so taking the test off it would save nothing.
