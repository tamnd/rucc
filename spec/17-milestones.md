...
**Exit:** all of rung 2 builds and passes its own suites at all levels on both targets; the matrix reports its coverage; the claim `-fgnuc-version=` defaults to is still the highest version measured to get a real header set and a real corpus through, which is now 16.0.0 and was raised there ahead of this milestone because micropython needed it rather than because the matrix reached a line.
