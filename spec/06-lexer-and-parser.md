...
Not every dialect difference is a keyword. `//` starts a comment in C99 and in every GNU dialect, and in `-std=c89` it is an error, said once a file and skipped as a comment anyway so that a file written with line comments produces one complaint rather than a syntax error on every other line. A declaration in a `for` header is C99 and GCC refuses it in gnu89 as well, which is one of the places a GNU dialect is not the iso one with more allowed; the header is parsed anyway so the body is still checked. Both of those and the digit separator are measured against GCC rather than read off the standard, because the question is which dialect GCC puts a feature in and not which one the committee did.

Numeric conversion is where a compiler quietly loses correctness. Integer constants are parsed into `u128` with overflow detected rather than wrapped, and the type is selected by the standard's table walk over the candidate list for the given suffix and base, with a diagnostic when no type fits. Floating constants are parsed with a correctly-rounded decimal-to-binary conversion, not `strtod` on the host, which would make output depend on the host's libc and break determinism, and not a naive `mantissa * 10^exp`, which is wrong in the last bit for a meaningful fraction of literals. Hexadecimal floating constants are exact by construction and are the easy case. C23 digit separators are stripped during conversion, and before C23 there is no such thing to strip: the apostrophe is part of a pp-number only in that dialect and is not a GNU extension, so `1'000'000` in a c17 program is a number, a multi-character character constant and another number, which is how GCC reads it and why it does not parse. `_BitInt` suffixes (`wb`, `uwb`) select the exact-width types.
...
Which dialect has which prefix is measured against GCC rather than read off the standard, because GCC offers the three C11 string prefixes as an extension from `gnu99` on and offers none of them in `gnu89`. The one it does not offer early is `u8` on a character constant, which is C23 in every dialect including `gnu23`. The element type of a `u8` string moves with the dialect as well: C23 gave the prefix the type `char8_t`, which in C is `unsigned char`, and before C23 the literal is an array of plain `char`. What that changes is whether `const char *p = u8"x"` is silent or says the two pointers differ in signedness.
...
`__builtin_offsetof`, `__builtin_choose_expr`, `__builtin_types_compatible_p` and the variable argument family are syntax rather than functions, their operands are types or unevaluated or objects rather than values, and so they are parsed here rather than treated as calls. `__builtin_choose_expr` in particular must not type-check the untaken branch, which is the entire reason it exists. The family is `__builtin_va_list`, which names a type, `__builtin_va_arg`, whose second operand is a type name, and `__builtin_va_start`, `__builtin_va_end` and `__builtin_va_copy`, which are handed the list itself so that its address can be taken, and whose second operand in the case of `va_start` names a parameter and is not evaluated.
