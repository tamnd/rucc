...
- `coverage::table` answers `None` for AArch64 even though the rule file exists, because a coverage report for a target nothing compiles for yet would be empty. It gets its arm when `Machine::for_target` returns AArch64.
